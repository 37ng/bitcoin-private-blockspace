...
-- The join reaches across datasets -- `a` from `${ms_dst}`, `b` from
-- `${dst}` -- and the result is written back into `${ms_dst}` with the
...
CREATE OR REPLACE TABLE `${ms_dst}.ms_by_pool` AS
SELECT
  b.pool_name,
...
-- Cancelled-but-completed accelerations count: see ms_monthly.sql.
