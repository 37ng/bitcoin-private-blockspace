-- Step 04d: the ancestor-limit flag, the one relay rule that needs the CPFP
...
-- child was the one too many, and an ambiguous case is not flagged.
--
...
--                                    single transaction. Nothing is flagged.
...
-- flags still lives in exactly one place.
UPDATE `${dst}.txs` AS t
SET
  flag_ancestor_limit = f.flag_ancestor_limit,
  is_nonrelayable = t.is_nonrelayable OR f.flag_ancestor_limit
FROM (
  SELECT
    p.tx_hash,
    p.block_number,
