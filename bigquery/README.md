...
03_txs           in-block CPFP edges, and the non-relayable flags
...
04d_ancestors    the one relay flag that needs the CPFP graph
...
date. A case the rule in force does not settle is not flagged at all.

| flag | rule | gate |
|---|---|---|
| `flag_nonstandard_script` | an output scriptPubKey matching no standard template | none |
| `flag_bare_multisig` | more than 3 pubkeys | none |
| `flag_op_return` | OP_RETURN scriptPubKey over 83 bytes, then over 100,000 summed across the outputs | 2025-10-08, Core v30 |
| `flag_multi_op_return` | more than one OP_RETURN output | until 2025-10-08, Core v30 |
| `flag_dust` | an output worth less than the input that would spend it | one is allowed from 2025-04-15 (Core 29) on a 0-fee parent whose child spends it |
| `flag_version` | version outside 1..2, then outside 1..3 | 2024-10-04, Core 28 |
| `flag_truc` | the version 3 size and version-mixing rules (10,000 vB, 1,000 vB for a child) | from 2024-10-04, Core 28 |
| `flag_sub_minrelay` | under 1 sat/vB, then under 0.1 | 2025-09-04, Core 29.1 |
| `flag_oversized` | virtual size over 100,000 vB (the 400k WU standard limit) | none |
| `flag_undersized` | under 65 non-witness bytes | none |
| `flag_scriptsig_size` | an input scriptSig over 1,650 bytes | none |
| `flag_scriptsig_nonpush` | an input scriptSig that opens with a real opcode | none |
| `flag_ancestor_limit` | more than 25 in-block ancestors, or more than 101,000 vB of them | until 2026-04-20, Core 31 |
...
names no single transaction, so `flag_ancestor_limit` never fires after
2026-04-20.
...
cannot be checked. Sigop cost is not computed. `flag_scriptsig_nonpush` only
...
standard: a missed flag understates non-relayable traffic, while a false one
...
`test_relay_flags.py` covers the relay rules, which is where a mistake costs
the most: a false flag deletes a relayable transaction from the measurement.
...
```bash
BQ_FIXTURES=1 uv run python -m pytest tests/test_relay_flags.py -q
```
